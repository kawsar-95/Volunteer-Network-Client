...
const EventTasks = () => {
  ...
  const loadEvents = () => {
    fetch(
      "https://hidden-eyrie-83298.herokuapp.com/events?email=" +
        loggedInUser.email,
      {
        method: "GET",
  ...
  const handleDelete = (id) => {
    fetch(`https://hidden-eyrie-83298.herokuapp.com/eventDelete/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((result) => {
          ...
          <>
            <div className="text-center mb-5">
              <h3 className="text-center">
                <span style={{ color: "#4f82f1" }}>{loggedInUser.name}</span>,
