...
const Admin = () => {
  ...
  const loadAllEvents = () => {
    // useEffect(() => {
    fetch("https://hidden-eyrie-83298.herokuapp.com/allEvents")
      .then((res) => res.json())
  ...
  const handleDelete = (id) => {
    fetch(`https://hidden-eyrie-83298.herokuapp.com/deleteUserEvent/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((result) => {
        ...
        <tbody>
          {allEvents.map((event) => (
            <tr className="text-center">
              ...
              <td onClick={() => handleDelete(event.id)}>
                <img style={style} className="p-1" src={removeIcon} alt="" />
              </td>
            </tr>
          ))}
        </tbody>
